...
namespace Apostol {
    ...
    namespace AWS {
        ...
        void CAWS::StartProcess() {
            ...
            if (Config()->IniFile().ReadBool("process/StreamServer", "enable", false)) {
...
int main(int argc, char *argv[]) {
    ...
    CAWS paneliot(argc, argv);
    
    try
    {
        paneliot.Name() = APP_NAME;
        paneliot.Description() = APP_DESCRIPTION;
        paneliot.Version() = APP_VERSION;
        paneliot.Title() = APP_VER;

        paneliot.Run();

        exitcode = paneliot.ExitCode();
    }
